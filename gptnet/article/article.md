...
Before applying softmax to attention scores, future positions are masked by setting their scores to **negative infinity**. When softmax is applied, `exp(-∞)` becomes 0, effectively removing future tokens from the attention computation.
...
| Hugging Face | C# Model |
|--------------|----------|
| `wte.weight` | `embeddings.wte.weight` |
| `h.0.ln_1.weight` | `blocks[0].ln1.weight` |
| `h.0.attn.c_attn.weight` | `blocks[0].attn.c_attn.weight` |
